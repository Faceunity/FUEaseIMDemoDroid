...
public class FURenderer extends IFURenderer {
     ...
     */
    @Override
    public byte[] onDrawFrameSingleInput(byte[] img, int width, int height) {
        prepareDrawFrame();
        ...
        if (outputData.getImage() != null && outputData.getImage().getBuffer() != null) {
            return outputData.getImage().getBuffer();
        }
        return inputData.getImageBuffer().getBuffer();
    }
     ...
     */
    @Override
    public byte[] onDrawFrameDoubleInput(int texId, byte[] img, int width, int height) {
        prepareDrawFrame();
        FURenderInputData inputData = new FURenderInputData(width, height);
        /*注释掉Buffer配置，启用单纹理模式，防止Buffer跟纹理存在不对齐造成，美妆偏移*/
     ...
     * 设置数据回写同步
     */
    public void setReadBackSync(boolean enable) {
        mFURenderKit.setReadBackSync(enable);
    }
